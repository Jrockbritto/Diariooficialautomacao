...
def get_all_links(driver):
    ...
    for i in range(1,len(elements)-1) : #eliminando cabeçalho e footer da table
        ...
        links.append('http://diariooficial.rn.gov.br/dei/dorn3/documentos/00000001/'+n+'/'+l+'.htm')
        titulo.append(elements[i].find_element_by_xpath('.//td[3]').get_attribute('innerHTML'))
        data.append(elements[i].find_element_by_xpath('.//td[4]').get_attribute('innerHTML'))
...
def elinput(Entrada, Xpath, web):
   ...
   web.find_element_by_xpath('//*[@id="Form1"]/section[2]/div').click() #Alterar foco
...
def start(P_chave, Data, hoje):
    ...
    time.sleep(2)
    elinput(P_chave,'//*[@id="input-bs-keyword"]', web) #Preenchendo palavra chave
    elinput(Data,'//*[@id="input-bs-data"]', web) #Preenchendo data inicio
    elinput(hoje,'//*[@id="input-bs-data-2"]', web)  #Preenchimento da tada final (dia em que o script roda)

    Submit = web.find_element_by_xpath('//*[@id="submit-busca-simples"]')
    Submit.click()

    page = web.find_element_by_xpath('//*[@id="lblPagina"]').get_attribute('innerHTML')[12:]
    pagina = 1
    while True:
...
def gerarExcel (links, titulo, data, datainicio, name):
      ...
      if os.path.isfile(os.path.join(path,name)):
        ...
        if linksunicos == [] :
            ...
            if increment and (ws.cell(1, 2, increment).value) :
                for row_num, dados in enumerate(linksunicos):
                    ws.cell(row_num+increment+1, 1, titulounicos[row_num]) #Rownum percorre as lista, enquanto increment move o começo para o ultimo elemento da lista, e o +1 para pular o ultimo elemento
        ...
        with xlsxwriter.Workbook('resultado' + '/' + name) as workbook:
            ...
            for row_num, dados in enumerate(links):
                ...
                worksheet.write(0, 1 , row_num + 2) #+1 por conta da mudança de coordenada (0:0) para (1:1) +1 para eliminar cabeçalho
                worksheet.write_string(0, 2 , titulo[row_num])
                worksheet.write_string(0, 3 , dados)
                worksheet.write_string(0, 4 , data[row_num])

def mkdir(linkslei, titulolei, datalei, datainicio, name) :
   cwd = os.getcwd()
   path = os.path.join(cwd,"resultado")
   if os.path.exists(path):
      if os.path.isdir(path):
         if linkslei == [] :
            ...
            gerarExcel(linkslei, titulolei, datalei, datainicio, name)
   else :
      if linkslei == [] :
         print ("Nao foram encontrados documentos novos nessa pesquisa")
      else :
...
if __name__ == '__main__':
    ...
    links, titulo, data, web = start("14.133", datainicio, date.today().strftime("%d/%m/%Y"))  # parametros: palavra de pesquisa e numero de pag pesquisadas 
